...
export function computeRemovals(metadata, mode = 'attribution') {
  ...
  for (const [fullKey, value] of Object.entries(metadata)) {
    ...
    if (isAttributionTag && META_PATTERN.test(strValue)) {
...
export async function generateReport(inputPath, outputPath, removals) {
  ...
  // Count preserved tags (excluding SourceFile and ExifTool metadata)
  const preservedCount = Object.keys(outputMeta).filter(
    (k) => k !== 'SourceFile' && !k.startsWith('ExifTool:'),
  ).length;
