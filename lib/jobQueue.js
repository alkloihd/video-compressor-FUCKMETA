...
export class JobQueue extends EventEmitter {
  constructor() {
    super();
    // Two lanes: HW is capped at the number of VideoToolbox encode engines (2 on M2 Max).
    // SW runs concurrently with HW so libx264/libx265 don't stall behind a hardware queue.
    this._hwQueue = new PQueue({ concurrency: 2 });
    this._swQueue = new PQueue({ concurrency: 3 });
    this._jobs = new Map();
  }
  ...
  addJob(jobSpec) {
    ...
    const allArgs = [...(jobSpec.pass1Args || []), ...(jobSpec.ffmpegArgs || [])];
    const isHw = allArgs.some((a) => typeof a === 'string' && a.includes('videotoolbox'));
    const queue = isHw ? this._hwQueue : this._swQueue;
    queue.add(() => this._runJob(id, jobSpec));
